...
public class TrialDialogueEditor : DREditor.DialogueEditor.Editor.DialogueEditorBase
{
    TrialDialogue dia;
    public void OnEnable()
    {
        dia = (TrialDialogue)target;
    }
    public override void OnInspectorGUI()
    {
            ...
            for (int i = 0; i < dia.Lines.Count; i++)
            {
                ...
                if (dia.Lines[i].SFX != null)
                {
                    for (var j = 0; j < dia.Lines[i].SFX.Count; j++)
                    {
                        ...
                        {
                            DREditor.Utility.PublicAudioUtil.PlayClip(dia.Lines[i].SFX[j]);
                        }
                ...
                dia.Lines[i].vfx = (eTrialVFX)EditorGUILayout.EnumPopup(dia.Lines[i].vfx);
                GUILayout.EndHorizontal();

                GUILayout.BeginHorizontal();
                GUILayout.Label("Cam:", GUILayout.Width(40));
                dia.Lines[i].camAnim = (eCamAnim)EditorGUILayout.EnumPopup(dia.Lines[i].camAnim);
                GUILayout.EndHorizontal();
        ...
        //        //    (TrialDialogue)EditorGUILayout.ObjectField(dia.Choices[i].NextDialogue, typeof(TrialDialogue), true,
        //        //        GUILayout.Width(100));
        //
        //        GUI.backgroundColor = dia.Color;
        //
        ...
        if (dia.Variable.Enabled)
        {
            ...
            //dia.Variable.NextDialogueTrue = (TrialDialogue)EditorGUILayout.ObjectField(dia.Variable.NextDialogueTrue, typeof(TrialDialogue), false, GUILayout.Width(200));
            ...
            //}
            //
            //dia.Variable.NextDialogueFalse = (TrialDialogue)EditorGUILayout.ObjectField(dia.Variable.NextDialogueFalse, typeof(TrialDialogue), false, GUILayout.Width(200));
        ...
        if (dia.DirectTo.Enabled)
        {
            ...
            //    (TrialDialogue)EditorGUILayout.ObjectField(dia.DirectTo.NewDialogue, typeof(TrialDialogue), false,
            //        GUILayout.Width(180));
            //GUI.backgroundColor = dia.Color;
            //
            //if (dia.DirectTo.NewDialogue != null)
        ...
        if (!dia.Variable.Enabled && !dia.DirectTo.Enabled && !dia.SceneTransition.Enabled)
        {
            if (dia.Speakers == null)
            {
                if (Resources.Load<CharacterDatabase>("Characters/CharacterDatabase"))
